import { Contract, LocalSigner, StellarAssetContract } from "@colibri/core";
...
const {
  bulkTransferId,
  gUsers: receiversSecrets,
  cUsers: senders,
} = await readFromJsonFile<DeploymentData>("deployment");

const admin: LocalSigner = LocalSigner.fromSecret(receiversSecrets[0]);
const receivers = receiversSecrets
  .slice(1)
  .map((secret) => LocalSigner.fromSecret(secret));
...
switch (fnName) {
  case functionNames.OneToOne: {
    const to = receivers.pop()?.publicKey();
    const from = senders.pop();
  ...
  case functionNames.OneToMany: {
    ...
    for (let i = 0; i < count; i++) {
      const u = receivers.pop()?.publicKey();
